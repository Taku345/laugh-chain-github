        ...
        <>
            <ul className="text-sm flex flex-wrap">
                {district.candidate.map((candidate) => (
                    <div
                        key={candidate.id}
                        className=" border rounded p-4 px-6 my-2 ml-1"
                    >
                        「{candidate.name}」
                    </div>
                ))}
            ...
            {auth.account &&
                district.progress ==
                    laugh_chain_config.district.progress.running && (
                    <>
                        <input
                            type="text"
                            value={data.name}
                        ...
                        <button
                            ...
                            disabled={processing}
                            className={` ${
                                processing ? "bg-slate:100" : ""
                            } rounded bg-slate-600 text-white font-bold py-2 px-10 my-2 text-lg`}
                        >
                            立候補
                        </button>
                        {errors.name && (
